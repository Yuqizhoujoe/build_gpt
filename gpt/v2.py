...
eval_interval = 500  # Frequency of performance evaluation
learning_rate = 1e-3  # Step size for optimizer
device = 'cuda' if torch.cuda.is_available() else 'cpu'  # Use GPU if available
eval_iters = 200  # Iterations for loss estimation
n_embd = 32  # Dimensionality of embeddings and hidden states
n_head = 6  # Number of attention heads
n_layer = 6  # Number of transformer blocks
dropout = 0.2  # Dropout rate for regularization

# Transformer Architecture
# Embedding layers convert tokens and positions to dense vectors
# Attention mechanism with multiple heads for diverse focus
# FeedForward network for further processing post-attention
...
class Head(nn.Module):
     ...
     def forward(self, x):
          ...
          # output if size (batch, time-step, head size)
          B,T,C = x.shape
          k = self.key(x) # (B,T,hs)
...
class FeedForward(nn.Module):
     ''' a simple linear layer followed by a non-lineraity '''
